...
app.get('/api/user/correct', function(req, res) {
	var users = [];
	for (key in user) {
		var flg = user[key].answer.forEach(function(num, i) {
			return num === defaultAnswer[i];
		});
		if (flg) {
			users.push(user[key]);
		}
...
io.sockets.on('connection', function (socket) {
	...
	socket.on('select', function() {
		var id = user[Object.keys(user)[select(user)]].id;
